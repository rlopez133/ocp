...
for HOSTNUM in 0 ; do
  ...
  --nic net-id=$netid2,v4-fixed-ip="172.18.20.3" \
  --security-group infra-node-sg --user-data=user-data/infra-node-${HOSTNUM}.yaml \
...
for HOSTNUM in 1 ; do
  ...
  --nic net-id=$netid2,v4-fixed-ip="172.18.20.4" \
  --security-group infra-node-sg --user-data=user-data/infra-node-${HOSTNUM}.yaml \
  --block-device-mapping vdb=${VOLUMEID} \
  infra-node-${HOSTNUM}.${DOMAIN}
done
